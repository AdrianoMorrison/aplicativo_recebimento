...
;WITH CTE AS (
		...
		SELECT DISTINCT
            ...
            ISNULL(CAST(MAX(C.DATA_HORA_INICIO) AS date), FORMAT(MAX(A.DATA_AGENDAMENTO),'yyyy-MM-dd')) AS DATA_AGENDAMENTO,
   ...
   --       	TF.DIA_ENTRADA_LOJA,
	...
	CROSS APPLY ( 
			SELECT TOP 1 TC.DIA_ENTRADA_LOJA
			FROM TEMP_COD TC
			WHERE TC.ORDEM_PRODUCAO = A.NUMERO_DOCUMENTO
			ORDER BY TC.DIA_ENTRADA_LOJA DESC
